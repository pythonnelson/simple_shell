...
/**
 * get_input - this get a line minus the newline
...
ssize_t get_input(info_t *info)
{
	...
	if (r == -1) /* EOF */
		return (-1);
	if (len) /* when we have commands left in the chain buffer */
	{
		j = i; /* init new iterator to current buf pos */
		p = buf + i; /* get pointer to be return */
 ...
 * @info: the parameter to be struct
 * @buf: the buffer
 * @i: the size
 *
 * Return: then return r
 */
 ...
 * @sig_num: the signal of the number
 *
 * Return: then returns void
 */
